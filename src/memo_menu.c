...
#define ITEMS_PER_PAGE 10          // 페이지당 아이템 수
#define CLIENT_MEMO_CACHE_SIZE 100 // 클라이언트 메모 캐시 크기
#define MAX_MEMOS_PER_PAGE 10      // 페이지당 최대 메모 수
...
// 함수 원형 선언
static void display_ui(int year, int month, int page);                              // 화면 표시
static void fetch_and_display_memos(SOCKET sock, const char *user_id, int page);    // 메모 데이터 가져오기 및 표시
static void view_memo_details(SOCKET sock, const char *user_id);                    // 메모 상세 보기
static void add_new_memo(SOCKET sock, const char *user_id);                         // 새 메모 추가
static void update_existing_memo(SOCKET sock, const char *user_id);                 // 기존 메모 수정
static void delete_existing_memo(SOCKET sock, const char *user_id);                 // 기존 메모 삭제
static bool prompt_and_execute_search(SOCKET sock, const char *user_id);            // 검색 실행
static void handle_download_process(SOCKET sock, const char *user_id, int memo_id); // 다운로드 처리
    ...
    while (true)
    {
        ...
        if (ch == 224 || ch == 0)
        {
            ...
            case 80: // Down (월 이동은 월별 모드에서만)
                ...
                break;
            case 75: // Left
                // 현재 페이지가 0보다 크면 페이지 감소
                if (current_page > 0)
                    current_page--;
                // 페이지 감소
                break;
            case 77: // Right
                // 현재 페이지가 총 페이지 수보다 작으면 페이지 증가
        ...
        else
        {
            ...
            case '1':
                ...
                needs_update = true; // 새 메모 추가 후, 화면을 새로고침
                break;
            case '2':
                update_existing_memo(sock, logged_in_id);
                needs_update = true; // 기존 메모 수정 후, 화면을 새로고침
                break;
            case '3':
                delete_existing_memo(sock, logged_in_id);
                needs_update = true; // 기존 메모 삭제 후, 화면을 새로고침
                break;
            case '4': // 월별 모드에서만 '검색'으로 동작
                if (g_view_mode == MODE_MONTHLY)
                {
                    ...
                    needs_update = true; // 검색 후, 화면을 새로고침
                }
                break;
            }
        }
